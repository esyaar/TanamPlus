...
// Definisikan tipe untuk chartData
interface ChartData {
  name: string;
  value: number;
  color: string;
...
// Definisikan tipe untuk groupedData
...
export default class Dashboard extends Component<{}, State> {
  state: State = {
    ...
    kecamatanList: [],
    selectedMonth: `${new Date().getFullYear()}-${(new Date().getMonth() + 1).toString().padStart(2, '0')}`, // Bulan saat ini (Juli 2025)
    monthList: [],
    chartData: [],
  };
  ...
  fetchChartData = async () => {
    try {
      ...
      lttData.forEach(data => {
        if (!data.komoditas || !data.luasTambahTanam || !data.tanggalLaporan) return; // Validasi data
        const date = data.tanggalLaporan;
        if (!(date instanceof Date && !isNaN(date.getTime()))) return; // Validasi tanggal
        const monthYear = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}`;
  ...
  handleDownload = async () => {
    try {
      ...
      lttData.forEach(data => {
        if (!data.kecamatan || !data.komoditas || !data.jenisLahan || !data.tanggalLaporan) return; // Validasi data
        const date = data.tanggalLaporan;
        if (!(date instanceof Date && !isNaN(date.getTime()))) return; // Validasi tanggal
        const monthYear = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}`;
        ...
        if (
          ...
          const key = `${data.kecamatan}:${data.komoditas}:${data.jenisLahan}`;
          if (groupedData[key]) {
